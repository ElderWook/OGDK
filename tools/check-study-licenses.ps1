...
# Usage: check-study-licenses.ps1 [-Write] [StudyDir]   (-Write emits study-repo\STUDY-INDEX.md)
# Twin: check-study-licenses.sh
param([switch]$Write, [string]$StudyDir)
$ErrorActionPreference = 'Stop'

$RepoRoot = Split-Path -Parent $PSScriptRoot
$DevRoot  = Split-Path -Parent $RepoRoot
if (-not $StudyDir) { $StudyDir = Join-Path $DevRoot 'study-repo/study' }
$Index = Join-Path $DevRoot 'study-repo/STUDY-INDEX.md'
...
function Folded([string]$n) {
    $files = @((Join-Path $RepoRoot 'LESSONS.md'), (Join-Path $RepoRoot 'LESSONS-ARCHIVE.md'), (Join-Path (Join-Path $DevRoot 'study-repo') 'STUDY-FOLD-IN-MAP.md'))
...
$md.Add('# Study index'); $md.Add('_Generated by `check-study-licenses.ps1 -Write` - do not edit by hand._'); $md.Add('')
$md.Add('| Clone | License | Class | Folded | Source |'); $md.Add('|---|---|---|---|---|')
...
foreach ($dir in (Get-ChildItem -LiteralPath $StudyDir -Directory)) {
    ...
    $md.Add("| $($dir.Name) | $id | $class | $fold | $src |")
    if ($fold -eq 'yes') {
...
if ($Write) {
    $md.Add(''); $md.Add("_$total clone(s) - $high strong-copyleft + $med weak/unknown have a fold-in. Strong-copyleft (GPL/AGPL) = ideas-only._")
