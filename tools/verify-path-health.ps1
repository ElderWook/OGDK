...
.USAGE
    ...
    Or add to your profile: . C:\OpenBook_Release\tools\verify-path-health.ps1
#>

Write-Host "======================================" -ForegroundColor Cyan
Write-Host "  PATH Health Check for OpenBook Dev  " -ForegroundColor Cyan
Write-Host "======================================" -ForegroundColor Cyan
Write-Host ""
...
# Check 6: OpenBook_Release not cloud-synced
$attr = (attrib 'C:\OpenBook_Release' 2>&1)
if ($attr -match '\bP\b|\bO\b') {
    Write-Host "[WARN] C:\OpenBook_Release may have cloud-sync attributes: $attr" -ForegroundColor Yellow
    $issues += "Cloud sync on project dir"
} else {
    Write-Host "[PASS] C:\OpenBook_Release has no cloud-sync overlay attributes" -ForegroundColor Green
}
