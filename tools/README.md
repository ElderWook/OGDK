...
| `check-study-licenses.{ps1,sh}` | (kit only, not propagated) **study license gate** — scans every clone under `../study-repo/study/` and classifies each license permissive / weak-copyleft (LGPL/MPL) / strong-copyleft (GPL/AGPL) / unknown (root LICENSE → SPDX `LICENSES/` REUSE strictest-wins → package manifest → README), then flags any copyleft/unknown source that already has a fold-in (name referenced in any LESSONS / STUDY-FOLD-IN-MAP). FAILs (exit 1) on strong-copyleft + fold-in — the "ideas-only, never port the algorithm" guard for repo-study. Run during a study sweep / before any code-shaped ADOPT. |
...
| `fleet-work.{ps1,sh}` | (kit only, not propagated) **read-only work rollup** — the WORK-state companion to `fleet-status` (which is git state). For every repo under the dev root, tallies OPEN lessons + active-plan count + the STATUS "Next up" line. `--write` emits `FLEET-BOARD.md` (the data-generated board; the hand-curated `FLEET-STUDY-SORT.html` stays the showcase). The "what is the fleet working on?" view; scales without hand-maintenance. |
