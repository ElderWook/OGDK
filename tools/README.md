...
| `install-hooks.{ps1,sh}` | point this clone's `core.hooksPath` at the tracked `tools/hooks/` so the **pre-push identity guard** (`tools/hooks/pre-push` runs `check-git-identity`) fires before every push. Per-clone, idempotent; undo with `git config --unset core.hooksPath`. The hook is a single LF-pinned sh script (git runs hooks through its own sh on both OSes); it calls the read-only `.sh` checker, so no NTFS-write hazard. |
