# LESSONS — ARCHIVE (cold storage for the OGDK kit learning loop)

> The **cold tier** of the lessons three-tier model (working buffer → this archive).
> `LESSONS.md` is the short OPEN process-friction buffer; once an entry is **CODIFIED**
> (turned into a rule / script / doc) it moves here so the buffer stays meaningful.
> Study **records** also live here — their findings are codified into kit templates or
> folded to project repos, and their license/provenance is the generated
> `study-repo/STUDY-INDEX.md` manifest (the durable source). Text is archived, never
> deleted (storage discipline). See `docs-template/workflow/SCALING.md`.

---

## Codified process lessons (2026-06-10 → 2026-06-14)
...
**What happened:** several projects across the fleet carried their `tools/*.sh` as non-executable (100644) in git, so each needed a manual `chmod +x` + commit before the scripts would run — a recurring per-project annoyance the fleet sweep surfaced. Two of the kit's OWN source scripts (`fleet-status.sh`, `safe-agent-push.sh`) were also committed 100644.
...
**What happened:** a freshly-scaffolded Python project's very first gate FAILED on an empty project — the project-checks step ran `python -m unittest discover`, which on **Python 3.12+** exits code **5** ("NO TESTS RAN") instead of 0, so a brand-new, test-less Python project can never pass its own gate.
...
**Proposed fix:** guard the test run behind "do tests exist yet" — `find src -name 'test_*.py'` (sh) / `Get-ChildItem -Recurse -Filter test_*.py` (ps1) — and only invoke unittest when at least one test file is present; otherwise print "(no tests yet)" and pass cleanly. Proven in the originating Python project's `tools/gate.{sh,ps1}`; graduate the guard into `tools/gate.template.{ps1,sh}` and fix the `user-notes.md` App line.
**Status:** CODIFIED 2026-06-14, gate.template.{sh,ps1} + user-notes.md + propagate-tools.{sh,ps1} + new-project.{sh,ps1}.

## 2026-06-14 Project .gitignore never ignored PRIVATE-MARKERS.list — markers could be committed
**What happened:** the kit gitignores `tools/PRIVATE-MARKERS.list`, `tools/TARGETS.list`, and `user-notes.local.md`, but the project gitignore TEMPLATES (app printf + game template) never did. So when an operator seeded private markers in a project and ran `git add -A`, the markers file was staged and **committed silently**. The pre-commit content scan can't catch it (it deliberately EXCLUDES PRIVATE-MARKERS.list from scanning, assuming it's gitignored), and projects don't even carry the pre-commit hook (only pre-push, which scans identity, not content). Surfaced when fleet-status flagged `?? tools/PRIVATE-MARKERS.list` in three repos while a fourth showed a marker file that wasn't dirty — i.e. already tracked.
**Root cause:** the privacy system's gitignore protection lived only in the KIT's `.gitignore`, never in the scaffolded project gitignores; nothing templated or propagated it. The scan's exclusion of PRIVATE-MARKERS.list assumes the file is gitignored — an assumption that silently failed in every project.
**Proposed fix:** add the three private entries to the app + game gitignore templates (`new-project.{ps1,sh}` + `gitignore.game.template`) and retro-fit every existing project's `.gitignore`; make `new-project` + `propagate-tools` COPY the kit's canonical PRIVATE-MARKERS.list into every project (gitignored, never committed) so the scans work fleet-wide from a single seeding. Remediate any already-committed markers with `git rm --cached`.
**Status:** CODIFIED 2026-06-14 — gitignore templates + all 7 project gitignores + new-project/propagate-tools markers distribution. Verified: check-kit-docs green, twins parse. Operator follow-up (in-flight): run the leak check (`git ls-files`) per repo + `git rm --cached` any tracked PRIVATE-MARKERS.list; consider a history scrub only if a repo that committed a marker will ever go public.
...
> templates and/or folded to project repos (see the kit-retro codification record above
> and `../STUDY-FOLD-IN-MAP.md`); license/class/fold-in status for each clone is the
> generated `../study-repo/STUDY-INDEX.md` manifest (the durable provenance source).
> Retained here as the detailed ADOPT/ADAPT/DECLINE rationale — not OPEN process-lessons.
> **Restriction reminder:** strong-copyleft (GPL/AGPL) sources are IDEAS-ONLY (never port
> implementation); enforced by `tools/check-study-licenses`.
...
**Status:** ARCHIVED 2026-06-14 — proposal (skills/inspect-dependency) retained as a backlog candidate (not yet built); provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — folded to app/APP-ARCHITECT.md (Local-first); `@oasis/local-first-sync` is a rule-of-two extraction candidate; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — folded to app/APP-ARCHITECT.md (Local-first); provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — codified to game/conventions/gas-and-ui.md; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — codified to docs-template/core/python-simulation.md; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — DECLINE recorded in game/conventions/gas-and-ui.md (kept C++/Blueprint stack); provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — IDEAS-ONLY (sverchok is GPL, strong-copyleft); folded to the relevant project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — folded to the CAN project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — folded to the edge-gateway project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — folded to the CAD/CAM project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — IDEAS-ONLY (Mycodo is GPL, strong-copyleft); folded to the sensor-gateway project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — folded to the relevant project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — IDEAS-ONLY (can-utils carries a GPL-2.0-only portion, strong-copyleft); folded to the CAN project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — IDEAS-ONLY (grbl is GPL, strong-copyleft); folded to the fabrication project repo; provenance: study-repo/STUDY-INDEX.md.
...
**Status:** ARCHIVED 2026-06-14 — folded to a game project (UE5 automation backlog); provenance: study-repo/STUDY-INDEX.md.
