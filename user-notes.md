...
| `fleet-status.ps1/.sh` | (OGDK only) **read-only** health sweep across ALL your repos at once (kit + `tools/TARGETS.list`): branch / ahead / behind / dirty / stash / state / KIT-VERSION in one table, `*` = needs attention. The git-lifecycle **C0** check — run it at the start of any multi-repo or propagation session so you fix tangles before propagating. Changes nothing |
| `fleet-work.ps1/.sh` | (OGDK only) **read-only work rollup** — fleet-status' work-state twin: OPEN lessons + active plans + STATUS "Next up" per repo. `--write`/`-Write` generates `FLEET-BOARD.md`. The "what's everyone working on" view |
| `track-projects.ps1/.sh` | (OGDK only) register projects into your gitignored `tools/TARGETS.list` (the fleet list). No args = auto-discover OGDK projects under `~/dev` (git repo + `tools/KIT-VERSION`); or pass paths / `--scan <dir>`. Idempotent. Run it once after cloning a project on a NEW machine so it joins fleet-status + propagate; projects from `new-project` are auto-registered |
